...
function enviarSaldo(idUsuario, saldoInicial) {
    var idUsuario = document.getElementById("idUsuario").textContent;
    var saldoInicial = document.getElementById("saldoInicial").textContent;
    
    console.log("VOY A ENVIAR EL ID: " + idUsuario);
	console.log("VOY A ENVIAR EL SALDO: " + saldoInicial);

    $.ajax({
        url: '/ruleta/actualizarSaldo',
        type: 'POST',
        data: JSON.stringify({ idUsuario: idUsuario, saldo: saldoInicial }), 
...
function barraProgreso() {
  ...
  var interval = 100; // Intervalo de tiempo para actualizar la barra de progreso (en milisegundos)
...
function calcularBeneficio() {
  ...
  if (diferencia > 0) {
    document.getElementById("num_beneficio").innerHTML = diferencia.toFixed(2);
...
function calientesFrios(numerosSalidos) {
  var numerosRepetidos = {};

  numerosSalidos.forEach(numero => {
    numerosRepetidos[numero] = (numerosRepetidos[numero] || 0) + 1;
  });

  for (let numero in numerosRepetidos) {
    if (numerosRepetidos[numero] > 1 && calientes.length < 5) {
      calientes.push(numero);
    } else if (numerosRepetidos[numero] === 1 && frios.length < 5) {
      frios.push(numero);
    }

    if (calientes.length === 5 && frios.length === 5) {
      break;
    }
  }

  console.log("Números Calientes:", calientes);
  console.log("Números Fríos:", frios);
  document.getElementById("calientes").innerHTML = calientes.join(', ');
  document.getElementById("frios").innerHTML = frios.join(', ');
}
